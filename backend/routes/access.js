...
router.get('/login/:user', async (req, res) => {
    ...
    await log(ip, '/access/login/', 'ACCESS', 200, 'CREATE', user + ' se úspěšně přihlásil do systému.');
...
router.get('/logout/:user', async (req, res) => {
    const ip = req.ip;
    const user = req.params.user;
    await log(ip, '/access/logout/', 'ACCESS', 200, 'CREATE', user + ' se úspěšně odhlásil ze systému.');
    return res.status(200).json({ message: 'hodnota v pořádku uložena.' });
});
